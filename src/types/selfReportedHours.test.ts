...
describe("selfReportedHours types", () => {
  describe("ActivityType enum", () => {
    it("should have correct values", () => {
      ...
      expect(ActivityType.ADMINISTRATIVE_SUPPORT).toBe(
        "administrative_support",
      );
      expect(ActivityType.PROFESSIONAL_TECHNICAL).toBe(
      ...
      expect(ActivityType.LEADERSHIP_COORDINATION).toBe(
      ...
      expect(ActivityType.TRANSPORTATION_DELIVERY).toBe(
      ...
      expect(ActivityType.ENVIRONMENTAL_STEWARDSHIP).toBe(
  ...
  describe("RejectionReason enum", () => {
    it("should have correct values", () => {
      ...
      expect(RejectionReason.ACTIVITY_NOT_RECOGNIZED).toBe(
        "activity_not_recognized",
      );
      expect(RejectionReason.VOLUNTEER_NOT_RECOGNIZED).toBe(
        "volunteer_not_recognized",
      );
      expect(RejectionReason.DESCRIPTION_INSUFFICIENT).toBe(
  ...
  describe("canRequestValidation", () => {
    it("should return true for unvalidated status with non-expired date and verified org", () => {
      ...
      expect(
        canRequestValidation(ValidationStatus.UNVALIDATED, recentDate, true),
      ).toBe(true);
    });
    ...
    it("should return true for rejected status with non-expired date and verified org", () => {
      const recentDate = new Date().toISOString().split("T")[0];
      expect(
        canRequestValidation(ValidationStatus.REJECTED, recentDate, true),
      ).toBe(true);
    });
    ...
    it("should return false for validated status", () => {
      const recentDate = new Date().toISOString().split("T")[0];
      expect(
        canRequestValidation(ValidationStatus.VALIDATED, recentDate, true),
      ).toBe(false);
    });
    ...
    it("should return false for pending status", () => {
      const recentDate = new Date().toISOString().split("T")[0];
      expect(
        canRequestValidation(ValidationStatus.PENDING, recentDate, true),
      ).toBe(false);
    });
    ...
    it("should return false when no verified org", () => {
      const recentDate = new Date().toISOString().split("T")[0];
      expect(
        canRequestValidation(ValidationStatus.UNVALIDATED, recentDate, false),
      ).toBe(false);
    });
    ...
    it("should return false for expired activity date", () => {
      ...
      const expiredString = expiredDate.toISOString().split("T")[0];
      expect(
        canRequestValidation(ValidationStatus.UNVALIDATED, expiredString, true),
      ).toBe(false);
    });
  });
});
