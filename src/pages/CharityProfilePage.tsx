...
import { AlertTriangle, Clock, MapPin, Building2, Lock, Users, Heart } from 'lucide-react';
...
import type { CharityProfile, CharityProfileStatus } from '@/types/charityProfile';
...
function UnclaimedBanner({ nominationsCount }: { nominationsCount: number }) {
  return (
    <div className="rounded-lg bg-amber-50 border border-amber-200 p-4 flex items-start gap-3">
      <AlertTriangle aria-hidden="true" className="h-5 w-5 text-amber-600 shrink-0 mt-0.5" />
      <div className="text-sm text-amber-800">
        <p className="font-medium">This profile has not been claimed.</p>
        <p className="mt-1">
          Information sourced from IRS public records.
          {nominationsCount > 0 && ` ${nominationsCount} donor${nominationsCount === 1 ? ' has' : 's have'} nominated this charity.`}
        </p>
...
function ClaimedPendingBanner() {
  return (
    <div className="rounded-lg bg-blue-50 border border-blue-200 p-4 flex items-start gap-3">
      <Clock aria-hidden="true" className="h-5 w-5 text-blue-600 shrink-0 mt-0.5" />
...
function getTabMessages(status: CharityProfileStatus) {
  if (status === 'unclaimed') {
    return {
      financials: 'Financial data will be available once this profile is claimed and verified.',
      volunteers: 'Volunteer opportunities will be available once this profile is claimed and verified.',
    };
  }
  ...
  if (notFound || !profile) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16">
        <Card hover={false} className="p-8 text-center">
          <Building2 aria-hidden="true" className="h-10 w-10 text-gray-300 mx-auto mb-4" />
          <p className="text-gray-600">We couldn&apos;t find a charity with this EIN.</p>
        </Card>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Tabs defaultValue="about">
            ...
            <TabsContent value="about">
              <div className="space-y-4 mt-4">
                {/* Mission */}
                <Card hover={false} className="p-5">
                  <h3 className="text-sm font-semibold text-gray-900 mb-2">Mission</h3>
                ...
                {/* Social proof */}
                <Card hover={false} className="p-5">
                  <h3 className="text-sm font-semibold text-gray-900 mb-3">Community Interest</h3>
                  <div className="flex gap-6 text-sm">
                    <div className="flex items-center gap-2 text-gray-600">
                      <Users aria-hidden="true" className="h-4 w-4 text-gray-400" />
                      <span>{profile.nominations_count} nomination{profile.nominations_count !== 1 ? 's' : ''}</span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-600">
                      <Heart aria-hidden="true" className="h-4 w-4 text-gray-400" />
                      <span>{profile.interested_donors_count} interested donor{profile.interested_donors_count !== 1 ? 's' : ''}</span>
                    </div>
                  </div>
                </Card>
              </div>
        ...
        <div className="space-y-4">
          <SidebarPlaceholder
            icon={<Heart aria-hidden="true" className="h-6 w-6 text-gray-300" />}
            message="Donation features coming soon."
          />
          <SidebarPlaceholder
            icon={<Users aria-hidden="true" className="h-6 w-6 text-gray-300" />}
            message="Nomination features coming soon."
          />
          {profile.status === 'unclaimed' && (
            <SidebarPlaceholder
              icon={<Building2 aria-hidden="true" className="h-6 w-6 text-gray-300" />}
