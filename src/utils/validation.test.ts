...
describe('validateAuthInput', () => {
  it('should not throw for valid email and password', () => {
    expect(() => validateAuthInput('user@example.com', 'password123')).not.toThrow();
  });

  it('should throw when email is invalid', () => {
    expect(() => validateAuthInput('bademail', 'password123')).toThrow(/valid email/i);
  });

  it('should throw when password is too short', () => {
    expect(() => validateAuthInput('user@example.com', 'short')).toThrow(/8 characters/i);
  });
});
