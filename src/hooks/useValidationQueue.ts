...
export function useValidationQueue(): UseValidationQueueReturn {
  ...
  const fetchQueue = useCallback(async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to fetch validation queue';
      setError(message);
      Logger.error('Error fetching validation queue', { error: err, profileId: profile.id });
    } finally {
      setLoading(false);
    }
  }, [profile?.id]);
  ...
  const approveRequest = useCallback(async (requestId: string): Promise<boolean> => {
    ...
    try {
      ...
      showToast('success', 'Approved', 'Volunteer hours validated successfully');
      await fetchQueue();
      return true;
    } catch (err) {
  ...
  const batchApprove = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (failedCount === 0) {
        showToast('success', 'Batch Complete', `Approved ${successCount} requests`);
      } else {
        showToast(
          'warning',
          'Partial Success',
  ...
  const batchReject = useCallback(async (
    ...
    try {
      ...
      if (failedCount === 0) {
        showToast('success', 'Batch Complete', `Rejected ${successCount} requests`);
      } else {
        showToast(
          'warning',
          'Partial Success',
