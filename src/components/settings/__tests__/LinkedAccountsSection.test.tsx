...
beforeEach(() => {
  ...
  mockSupabase.auth.linkIdentity = jest.fn().mockResolvedValue({ data: null, error: null });
  // @ts-ignore
  mockSupabase.auth.unlinkIdentity = jest.fn().mockResolvedValue({ data: null, error: null });
...
describe('LinkedAccountsSection', () => {
  describe('rendering', () => {
    ...
    it('shows load error when getUser fails', async () => {
      ...
      renderSection();
      await waitFor(() => {
        expect(screen.getByText('Could not load linked accounts')).toBeInTheDocument();
      });
    });
  });
