...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body.message.includes(body.nonce)) {
      return jsonResponse(
        { success: false, error: "Invalid nonce in message" },
        401,
      );
    }
    ...
    if (!timestampMatch) {
      return jsonResponse(
        { success: false, error: "Missing timestamp in message" },
        401,
      );
    }
    const messageTime = new Date(timestampMatch[1]).getTime();
    if (Number.isNaN(messageTime)) {
      return jsonResponse(
        { success: false, error: "Invalid timestamp in message" },
        401,
      );
    }
    const fiveMinutesMs = 5 * 60 * 1000;
    if (Date.now() - messageTime > fiveMinutesMs) {
      return jsonResponse(
        {
          success: false,
          error: "Signature has expired. Please sign in again.",
        },
        401,
      );
    }
    ...
    if (verifyError || !sessionData.session) {
      console.error("Failed to verify OTP:", verifyError);
      return jsonResponse(
        { success: false, error: "Failed to establish session" },
        500,
      );
    }
